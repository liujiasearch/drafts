...
逻辑上，采用对局样本对结构二的神经网络进行训练似乎不太可行。我们的样本中针对任意局面仅有一个着法选项，而从网络结构上，我们是期望针对单独一个盘面可以有全部可落子点的样本标签，这显然是不可能做到的，它就相当于一盘棋穷举了全部的可能性。神经网络的优势在于它可以依赖有限的样本数据来猜测待拟合函数的样貌，而且在样本范围内常常做得很好。粗略地说，在训练了大量不同盘面的样本和标签的匹配关系后，我们默认同一盘面下的没有出现过的选项作为输入，神经网络依然可以达到较高的匹配度，虽然我们似乎无法验证这一点。
