...
4. 使用我们之前编写的棋盘类来记录游戏；
5. 如果不使用智能程序，我们还希望程序能够支持手工输入GTP命令，这个功能更多的是方便调试；
6. 让智能体根据当前棋局选择落子；
7. 把智能体的落子情况通知Gnu Go；
8. 请求Gno Go落子；
9. 更新当前棋局的状态记录；
...
现在我们已经具备了调用围棋引擎来下棋的能力，后面的章节中，我们会使用这个技术来生成大量的围棋对局。机器学习需要大量的样本，获取人类的对战棋局有一是数量有限，二是对局的水平参差不一。如果我们可以自己生成棋局记录，学习样本的质量和数量问题就可以得到解决。
