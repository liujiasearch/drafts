...
在智能程序互弈时，策略网络的下棋方法和梯度策略中使用的是一样的，我们按照概率比例从策略中选择着法，训练过程也是和策略梯度的训练方法一样。价值网络在下棋的过程中不起作用，我们仅保存价值网络的输出值，以备后续训练时知道策略网络的学习时使用。价值网络的训练过程比较简单，我们输入棋局，并将输出与最终棋局的实际胜负做比较，利用均方误差做反向梯度计算。具体的过程就不在本章再重复了，读者可以复习前两章中关于它们各自的内容，也可以直接阅读源码（`myGO/actor_critic/a_c.py`），它在实现上和之前的两种算法是非常相近的。
