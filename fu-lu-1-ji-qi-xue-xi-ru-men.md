...
如果去掉快捷连接，残差块就是一个普通的2层网络罢了。增加了快捷连接，使得梯度在反向传播时无需通过被绕过的那些网络结构即可直接向后传播。这种操作相当于在网络的浅层处添加了与其并列的网络结构，即使网络深度增加，其训练误差也不会高于原来的浅层网络。实际应用中，为了增加网络的深度，常常是采用多个残差块相互串联。
