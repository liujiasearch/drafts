...
7. 逐一计算参数 $$\theta_i$$ 的梯度 $$\frac{\partial L}{\partial \theta_i}$$ ;
...
神经网络反向传播和上面提到的梯度下降算法本质上是一样的。只是神经网络由于参与的节点太多，手工计算很容易出错，我们必须借助计算机工程软件才能将其应用到实践中。另外由于神经网络是由多个神经元拼接组成的，计算各个参数的梯度时必须得从输出层开始向输入层的方向逐级更新，这个顺序与计算系统预测输出的过程相反，反向传播也由此得名。我们可以认为在寻找神经网络最优解的时候，损失函数和神经网络是连接在一起的，通过损失函数作为媒介，逐级逆向求解神经网络内各层参数的梯度并更新。在使用网络进行预测时再将损失函数拆走。
...
正则化，drop，epoch保存，早停，训练集，测试集，验证集合
